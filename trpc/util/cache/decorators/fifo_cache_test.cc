...
/// @brief Test move semantics of FIFOCache.
...
TEST(FIFOCacheTest, MoveSemantics) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  FIFOCache<int, int> fifo_cache1(std::move(basic_cache), 4);
  fifo_cache1.Put(1, 1);
  ASSERT_EQ(fifo_cache1.Get(1), 1);

  FIFOCache<int, int> fifo_cache2 = std::move(fifo_cache1);
  ASSERT_EQ(fifo_cache2.Get(1), 1);
  ASSERT_EQ(fifo_cache2.Size(), 1);
}
...
TEST(FIFOCacheTest, BasicPutGetSize) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  FIFOCache<int, int> fifo_cache(std::move(basic_cache), 4);
...
TEST(FIFOCacheTest, PutDuplicateKey) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  FIFOCache<int, int> fifo_cache(std::move(basic_cache), 2);
...
/// @note Verifies that oldest items are evicted according to FIFO policy when cache reaches capacity.
TEST(FIFOCacheTest, EvictOldestKeyWhenFull) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  FIFOCache<int, int> fifo_cache(std::move(basic_cache), 3);
  ...
  ASSERT_EQ(fifo_cache.Get(1), std::nullopt);
  ASSERT_EQ(fifo_cache.Get(2), 2);
  ASSERT_EQ(fifo_cache.Get(3), 3);
  ASSERT_EQ(fifo_cache.Get(4), 4);
...
TEST(FIFOCacheTest, ConcurrentPut) {
  ...
  ASSERT_EQ(hits, fifo_cache.Size());
  ASSERT_EQ(misses, puts_num - fifo_cache.Size());
}
