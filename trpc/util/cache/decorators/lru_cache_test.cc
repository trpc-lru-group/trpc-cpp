...
/// @brief Test move semantics of LRUCache.
/// @note Verifies that move construction and move assignment correctly transfer ownership
///       and maintain cache consistency during object relocation.
TEST(LRUCacheTest, MoveSemantics) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  LRUCache<int, int> lru_cache1(std::move(basic_cache), 4);

  lru_cache1.Put(1, 1);
  ASSERT_EQ(lru_cache1.Get(1), 1);

  LRUCache<int, int> lru_cache2 = std::move(lru_cache1);
  ASSERT_EQ(lru_cache2.Get(1), 1);
  ASSERT_EQ(lru_cache2.Size(), 1);
}
...
TEST(LRUCacheTest, BasicPutGetSize) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  LRUCache<int, int> lru_cache(std::move(basic_cache), 4);
...
TEST(LRUCacheTest, PutDuplicateKey) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  LRUCache<int, int> lru_cache(std::move(basic_cache), 2);
...
TEST(LRUCacheTest, EvictLeastRecentlyUsedKeyWhenFull) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  LRUCache<int, int> lru_cache(std::move(basic_cache), 3);
...
/// @brief Test LRU access order maintenance.
/// @note Verifies that access order is correctly maintained and updated on each Get operation.
TEST(LRUCacheTest, AccessOrderMaintenance) {
  auto basic_cache = std::make_unique<BasicCache<int, int>>();
  LRUCache<int, int> lru_cache(std::move(basic_cache), 3);
  ...
  // Initial access order: 3 (most recent), 2, 1 (oldest)

  // Access key 1 - makes it most recent: 1, 3, 2
  ASSERT_EQ(lru_cache.Get(1), 1);

  // Access key 2 - makes it most recent: 2, 1, 3
  ASSERT_EQ(lru_cache.Get(2), 2);

  // Insert key 4 - should evict least recently used (key 3)
  lru_cache.Put(4, 4);

  // Verify eviction
  ASSERT_EQ(lru_cache.Get(3), std::nullopt);  // Least recently used (key 3) should be evicted
  ASSERT_EQ(lru_cache.Get(1), 1);             // Should remain
  ASSERT_EQ(lru_cache.Get(2), 2);             // Should remain
  ASSERT_EQ(lru_cache.Get(4), 4);             // Should remain
  ASSERT_EQ(lru_cache.Size(), 3);
}
...
TEST(LRUCacheTest, ConcurrentPut) {
  ...
  ASSERT_EQ(hits, lru_cache.Size());
  ASSERT_EQ(misses, puts_num - lru_cache.Size());
}
